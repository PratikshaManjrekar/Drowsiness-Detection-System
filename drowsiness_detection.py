...
while True: 
    ...
    frame = cv2.putText(frame, datet, (10, 70), font, 1,
    ...
    for (i, rect) in enumerate(rects): 
        ...
        # count the number of frames for which the eye remains closed 
        if EAR < EAR_THRESHOLD: 
            ...
            if FRAME_COUNT >= CONSECUTIVE_FRAMES: 
                count_sleep += 1
                # Add the frame to the datastore ar a proof of drowsy driving
                cv2.imwrite("datastore/frame_sleep%d.jpg" % count_sleep, frame)
        ...
        # Checking  if the person is yawning
        if MAR > MAR_THRESHOLD:
            count_yawn += 1
            cv2.drawContours(frame, [mouth], -1, (0, 0, 255), 1) 
            cv2.putText(frame, "DROWSINESS ALERT!", (270, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            # Add the frame to the dataset ar a proof of drowsy driving
            cv2.imwrite("datastore/frame_yawn%d.jpg" % count_yawn, frame)
            playsound('sound files/alarm.mp3')
            playsound('sound files/warning_yawn.mp3')
    #function for total data collection for plotting
    for i in ear_list:
        total_ear.append(i)
    for i in mar_list:
        total_mar.append(i)			
...
#Functions to print the date in csv 
dateti = str(datetime.date.today())

df = pd.DataFrame({"EAR" : a, "MAR":b,"TIME" : c})
df.to_csv("Results"+dateti+".csv", index=False)
df=pd.read_csv("Results"+dateti+".csv" )

df.plot(x='TIME',y=['EAR','MAR'])


#PLOT THE EAR VS MAR GRAPH
